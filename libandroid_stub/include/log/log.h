...
inline void process_format(std::stringstream &ss, const std::string_view &fmt) {
  for (const auto &c : fmt) {
    if (c == '%') {
      throw std::invalid_argument("Too few arguments provided for format specifiers.");
    }
    ss << c;
  }
}
...
void process_format(std::stringstream &ss, const std::string_view &fmt, T &&value, Args &&...args) {
  ...
  if (!found_placeholder) {
    throw std::invalid_argument("Too many arguments provided for format specifiers.");
  }
}

// Main formatting function
template <typename... Args>
std::string _make_str(const std::string &filename, int line, const std::string_view fmt,
                      Args &&...args) {
  std::stringstream ss;
  ss << "[" << filename << ":" << line << "] ";
