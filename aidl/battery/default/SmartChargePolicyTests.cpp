...
int main() {
  ...
  expect(EvaluateChargePolicy(80, 80, 70, true) ==
             ChargeDecision::DISALLOW_CHARGING,
         "upper edge should disable charging");
  expect(EvaluateChargePolicy(70, 80, 70, true) ==
             ChargeDecision::ALLOW_CHARGING,
         "lower edge should enable charging");
  expect(EvaluateChargePolicy(75, 80, 70, true) ==
             ChargeDecision::KEEP_CURRENT,
         "hysteresis middle should preserve state");
  expect(EvaluateChargePolicy(79, 80, -1, false) ==
             ChargeDecision::ALLOW_CHARGING,
         "stop-only mode should allow below upper");
  expect(EvaluateChargePolicy(-1, 80, 70, true) ==
             ChargeDecision::ALLOW_CHARGING,
  ...
  expect(!ParseIntegerPair("70garbage,80", &first, &second),
         "trailing garbage must be rejected");
  expect(!ParseIntegerPair("70,80,90", &first, &second),
         "multiple commas must be rejected");
