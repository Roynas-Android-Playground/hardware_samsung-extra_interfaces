...
int main() {
  ...
  const auto legacyOn = DecodeFlashlightValue(-1, 4);
  expect(legacyOn && legacyOn->enabled && legacyOn->brightnessLevel == 4,
         "legacy enabled state should use remembered brightness");
